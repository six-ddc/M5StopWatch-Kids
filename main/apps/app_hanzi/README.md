...
- 检索：**字母拨盘**拼音输入（`main/apps/common/pinyin_ime/`，可复用组件）——26 个字母沿表盘一圈、逐字母精确输入，没有 T9 数字歧义层。接不下去的字母原地变暗且不参与命中（触点吸附最近的亮字母，越往后目标越大）；按住沿环滑动有放大镜浮起、每跨一格短震一记；候选按"整音节命中在前、前缀展开在后"排在盘心 5+4 蜂窝网格里
...
| 点击 | 拨盘选字母 / 选字 → 学习页 | 选字 → 学习页 | 重播动画 |
| 删除 | 重按刚输入的字母（带圆点标记）/ 点拼音回显；长按回显清空 | — | — |
| 横滑 | —（环上滑动与横滑冲突，浏览入口暂时下线） | → 搜索页 | — |
| A+B 长按 | 退出 App | 返回搜索页 | 返回来处（搜索或浏览，再按退出）|

搜索交互为 5-8 岁设计：每输一个字母即时出字；接不下去的字母根本点不中，孩子永远不会进入"没有结果"的状态；"重按上一个字母 = 收回"依赖拼音无相邻重复字母这一事实（`hanzi_logic_test` 有断言钉死），被删字母还会沿弧飞回环上老家；多音字全部展开进索引（好 = hǎo/hào 都能查到），候选注音显示与当前输入匹配的那个读音。从学习页返回时搜索输入原样保留（孩子常连看同音字）。
...
view/         LVGL 页面，PSRAM 缓冲的所有权在这一层
  search      SearchPage：把可复用的字母拨盘组件接上笔顺引擎的薄胶水层
...
../common/pinyin_ime/   可复用 T9 拼音输入组件（不含任何 hz 代码）
  ...
  dial_view      LVGL 字母拨盘环+盘心候选网格；只认 CandidateSource（查询）与
