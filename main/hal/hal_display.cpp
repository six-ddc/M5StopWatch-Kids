...
public:
    ...
    bool init_impl(bool use_reset, bool use_clear) override
    {
        ...
        // The software framebuffer is REQUIRED: direct panel writes garble
        // the CO5300 (verified 2026-08-08 - boot screen corrupts without it).
        // Its cost: every LVGL flush is a ~7 Mpx/s CPU copy into PSRAM.
...
void hal_display_flush_stats(uint64_t &us, uint32_t &px, uint32_t &calls);

static void lvgl_rtos_task(void *pvParameter)
{
    (void)pvParameter;
    // simple render profiler: logs handler cost and effective fps once per second
    uint32_t stat_start_ms = 0;
    uint32_t handler_calls = 0;
    uint64_t handler_us    = 0;
    uint64_t handler_max   = 0;
    uint32_t heavy_calls   = 0;  // calls that actually rendered something (>3ms)
    while (1) {
        if (_lvgl_update_enabled && pdTRUE == xSemaphoreTake(xGuiSemaphore, portMAX_DELAY)) {
            ...
            if (stat_start_ms == 0) {
                stat_start_ms = now;
            } else if (now - stat_start_ms >= 1000) {
                uint64_t fus;
                uint32_t fpx, fcalls;
                hal_display_flush_stats(fus, fpx, fcalls);
                mclog::tagInfo("lv-perf", "fps~{} handler avg {}us max {}us calls {} | flush {}us {}px {}c",
                               heavy_calls, (uint32_t)(handler_us / handler_calls), (uint32_t)handler_max,
                               handler_calls, (uint32_t)fus, fpx, fcalls);
                stat_start_ms = now;
                handler_calls = 0;
                handler_us    = 0;
                handler_max   = 0;
...
// flush profiler: separates "CPU render" from "push to display" per second
static uint64_t _flush_us   = 0;
static uint32_t _flush_px   = 0;
static uint32_t _flush_call = 0;
void hal_display_flush_stats(uint64_t &us, uint32_t &px, uint32_t &calls)
{
    us  = _flush_us;
    px  = _flush_px;
...
static void lvgl_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
    ...
    if (esp_ptr_dma_capable(px_map)) {
        // Internal DMA-capable render buffer: let GDMA push the strip. This
        // both saturates the QSPI bus and bypasses the problematic SIMD copy
        // path entirely.
        gfx.writePixelsDMA((const lgfx::rgb565_t *)px_map, pixels);
        gfx.endWrite();
        _flush_us += esp_timer_get_time() - t0;
        _flush_px += pixels;
        _flush_call++;
...
void Hal::lvgl_init()
{
    ...
    // Prefer internal DMA-capable RAM: LVGL renders into it far faster than
    // into PSRAM, and the flush callback can hand it straight to GDMA.
    const size_t lv_buf_size = _display->width() * LV_BUFFER_LINE;
    static uint8_t *buf1 = (uint8_t *)heap_caps_malloc(lv_buf_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA);
    static uint8_t *buf2 = (uint8_t *)heap_caps_malloc(lv_buf_size, MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA);
    if (buf1 == NULL || buf2 == NULL) {
        mclog::tagWarn(_tag, "internal lvgl buffers unavailable, falling back to PSRAM");
        heap_caps_free(buf1);
        heap_caps_free(buf2);
        buf1 = (uint8_t *)heap_caps_malloc(lv_buf_size, MALLOC_CAP_SPIRAM);
        buf2 = (uint8_t *)heap_caps_malloc(lv_buf_size, MALLOC_CAP_SPIRAM);
    }
    lv_display_set_buffers(disp, (void *)buf1, (void *)buf2, lv_buf_size, LV_DISPLAY_RENDER_MODE_PARTIAL);
